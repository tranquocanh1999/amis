import Vue from 'vue'

import DxDropDownButton from 'devextreme-vue/drop-down-button';

Vue.component('DxDropDownButton', DxDropDownButton)
import { DxPager, DxFilterRow, DxScrolling, DxRowDragging, DxSorting, DxColumnChooser, DxSelection, DxDataGrid, DxColumn, DxEditing, DxPaging, DxLookup } from 'devextreme-vue/data-grid';
Vue.component('DxDataGrid', DxDataGrid)
Vue.component('DxColumn', DxColumn)
Vue.component('DxEditing', DxEditing)
Vue.component('DxPaging', DxPaging)
Vue.component('DxLookup', DxLookup)
Vue.component('DxSelection', DxSelection)
Vue.component('DxColumnChooser', DxColumnChooser)
Vue.component('DxRowDragging', DxRowDragging)
Vue.component('DxFilterRow', DxFilterRow)
Vue.component('DxPager', DxPager)
Vue.component('DxScrolling', DxScrolling)
Vue.component('DxSorting', DxSorting)
import DxSelectBox from 'devextreme-vue/select-box';
Vue.component('DxSelectBox', DxSelectBox)
import { DxPopover } from 'devextreme-vue/popover';
Vue.component('DxPopover', DxPopover)
import DxTextBox from 'devextreme-vue/text-box';
Vue.component('DxTextBox', DxTextBox)
import { DxCheckBox } from 'devextreme-vue/check-box';
Vue.component('DxCheckBox', DxCheckBox)
import { DxPopup } from 'devextreme-vue/popup';
Vue.component('DxPopup', DxPopup)
import DxDateBox from 'devextreme-vue/date-box';
Vue.component('DxDateBox', DxDateBox)
import DxTextArea from 'devextreme-vue/text-area';
Vue.component('DxTextArea', DxTextArea)
import DxTagBox from 'devextreme-vue/tag-box';
Vue.component('DxTagBox', DxTagBox)
import { DxTooltip } from 'devextreme-vue/tooltip';
Vue.component('DxTooltip', DxTooltip)
import {
    DxValidator,
    DxRequiredRule,
    DxCompareRule,
    DxEmailRule,
    DxPatternRule,
    DxStringLengthRule,
    DxRangeRule,
    DxAsyncRule
} from 'devextreme-vue/validator';
Vue.component('DxValidator', DxValidator)
Vue.component('DxRequiredRule', DxRequiredRule)
Vue.component('DxEmailRule', DxEmailRule)
Vue.component('DxCompareRule', DxCompareRule)
Vue.component('DxPatternRule', DxPatternRule)
Vue.component('DxStringLengthRule', DxStringLengthRule)
Vue.component('DxAsyncRule', DxAsyncRule)
Vue.component('DxRangeRule', DxRangeRule)